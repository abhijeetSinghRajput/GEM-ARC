  ...
  <div className={`bg-white shadow-lg ${isCollapsed ? 'w-20' : 'w-64'} transition-all duration-300 p-4 h-screen sticky top-0 z-20 hidden md:flex flex-col justify-between`}>
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {!isCollapsed && (
          <div className="flex-1 text-center">
            <h2 className="text-2xl whitespace-nowrap font-extrabold tracking-wide bg-gradient-to-r from-blue-600 to-indigo-500 bg-clip-text text-transparent">
              GEM-ARC
            </h2>
      ...
      <nav className="space-y-2">
        <Link to="/dashboard" className={`${isCollapsed && 'h-12 justify-center'} whitespace-nowrap flex items-center space-x-3 p-2 rounded-lg transition-all ${isActive('/dashboard') ? 'bg-blue-200 text-blue-800 shadow-sm' : 'hover:bg-blue-100'}`}>
          <Home size={20} className={isActive('/dashboard') ? 'text-blue-600' : 'text-gray-600'} />
          {!isCollapsed && <span className="font-medium">Home</span>}
        </Link>
        <Link to="/profile" className={`${isCollapsed && 'h-12 justify-center'} whitespace-nowrap flex items-center space-x-3 p-2 rounded-lg transition-all ${isActive('/profile') ? 'bg-blue-200 text-blue-800 shadow-sm' : 'hover:bg-blue-100'}`}>
          <User size={20} className={isActive('/profile') ? 'text-blue-600' : 'text-gray-600'} />
          {!isCollapsed && <span className="font-medium">Profile</span>}
        </Link>
        <Link to="/leaderboard" className={`${isCollapsed && 'h-12 justify-center'} whitespace-nowrap flex items-center space-x-3 p-2 rounded-lg transition-all ${isActive('/leaderboard') ? 'bg-blue-200 text-blue-800 shadow-sm' : 'hover:bg-blue-100'}`}>
        ...
        <Link to="/feedback" className={`${isCollapsed && 'h-12 justify-center'} whitespace-nowrap flex items-center space-x-3 p-2 rounded-lg transition-all ${isActive('/feedback') ? 'bg-blue-200 text-blue-800 shadow-sm' : 'hover:bg-blue-100'}`}>
        ...
        <Link to="/propose-event" className={`${isCollapsed && 'h-12 justify-center'} whitespace-nowrap flex items-center space-x-3 p-2 rounded-lg transition-all ${isActive('/propose-event') ? 'bg-blue-200 text-blue-800 shadow-sm' : 'hover:bg-blue-100'}`}>
        ...
        <Link to="/my-proposals" className={`${isCollapsed && 'h-12 justify-center'} whitespace-nowrap flex items-center p-2 rounded-lg transition-all ${isActive('/my-proposals') ? 'bg-blue-200 text-blue-800 shadow-sm' : 'hover:bg-blue-100'}`}>
          ...
          <div className="bg-gradient-to-r from-blue-100 to-indigo-100 rounded-lg p-3 shadow-md">
            <div className="text-center">
              <div className="inline-block bg-white rounded-full p-2 mb-2 shadow-sm">
                <div className="bg-gradient-to-r whitespace-nowrap from-blue-600 to-indigo-600 bg-clip-text text-transparent font-black text-2xl tracking-tight">
                  GEM-ARC
                </div>
              </div>
