...
function Leaderboard() {
  ...
  useEffect(() => {
    ...
    const fetchLeaderboard = async () => {
      try {
        ...
        const { data: leaderboardData } = leaderboardResponse;

        // Filter out admin users from the leaderboard
        const filteredUsers = leaderboardData.allUsers.filter(
          (user) => !user.isAdmin
        );
  ...
  const getBadge = (rank) => {
    if (rank === 1)
      return { icon: "👑", color: "text-yellow-500", title: "Champion" };
    if (rank === 2)
      return { icon: "🥈", color: "text-gray-400", title: "Silver Elite" };
    if (rank === 3)
      return { icon: "🥉", color: "text-amber-700", title: "Bronze Star" };
    if (rank <= 10)
      return { icon: "⭐", color: "text-blue-500", title: "Rising Star" };
                ...
                <div className="flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-blue-100 mt-2">
                      As an admin, you can view the leaderboard but your name is
                ...
                {allUsers.map((user, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className={`col-span-3 flex items-center ${badge.color}`}
                      >
