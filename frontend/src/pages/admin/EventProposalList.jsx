...
const EventProposalList = () => {
  ...
  const fetchProposals = async () => {
    try {
      const { data } = await api.get("/admin/event-proposals/all");
      ...
      setProposals(
        proposals.map((p) =>
          p._id === selectedProposal._id
            ? { ...p, status: "approved", adminFeedback }
            : p
        )
      );
  ...
  const handleReject = async () => {
    ...
    setProcessing(true);
    try {
      await api.put(`/admin/event-proposals/reject/${selectedProposal._id}`, {
        adminFeedback,
      });
      ...
      setProposals(
        proposals.map((p) =>
          p._id === selectedProposal._id
            ? { ...p, status: "rejected", adminFeedback }
            : p
        )
      );
  ...
  if (loading)
    return (
      <AdminLayout>
        <div className="text-center py-20">Loading proposals...</div>
      </AdminLayout>
    );
  if (error)
    return (
