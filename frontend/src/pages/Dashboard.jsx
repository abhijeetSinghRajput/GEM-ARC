  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const personalizedResponse = await api.get("/events/userRecommended", {
          headers: {
          ...
          <div className="animate-pulse space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1, 2, 3].map((_, i) => (
                <div
                  key={i}
                  className="h-64 bg-white rounded-xl shadow-md opacity-60"
                />
              ))}
            </div>
          </div>
            ...
            <div className="mb-12">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                <h2 className="text-2xl font-bold text-gray-800 flex items-center">
                  {showAllEvents
                    ? "All Events"
                    : categoryInfo[activeCategory]?.name || "Events"}
                  ...
                  eventsToDisplay.map((event) => (
                    ...
                    >
                      <EventCard
                        event={event}
                        categoryInfo={
                          !showAllEvents && event.recommendation
                            ? {
                                ...categoryInfo[
                                  event.recommendation.primaryCategory ||
                                    "personalized"
                                ],
              ...
              {eventsToDisplay.length > 0 && (
                <div className="mt-12 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl shadow-lg p-6 text-white text-center">
                  ...
                  <p className="text-indigo-100 mb-4">
                    Share your knowledge, build your network, and grow your
                    community.
                  </p>
                  <button
